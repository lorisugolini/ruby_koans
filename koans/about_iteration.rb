...
class AboutIteration < Neo::Koan
  ...
  def test_select_selects_certain_items_from_an_array
    ...
    # UGL aggiunto io
    assert_equal [], array.find_all { |item| item > 10 }
  end
  ...
  def test_find_locates_the_first_element_matching_a_criteria
    ...
    # UGL aggiunto io
    assert_equal nil, array.find { |item| item.size > 40 }
    end
  ...
  def test_inject_will_blow_your_mind
    ...
    # UGL aggiunto io
    result3 = [2, 3, 4].inject(1) { |sum, item| sum + item }
    assert_equal 1+2+3+4, result3

    result3 = [2, 3, 4].inject(10) { |sum, item| sum + item }
    assert_equal 10+2+3+4, result3

    result3 = [2, 3, 4].inject(10) { |sub, item| sub - item }
    assert_equal 10-2-3-4, result3

    # Extra Credit:
    # Describe in your own words what inject does.
    # UGL: per ogni item fa applica il blocco, per il primo item
    #      usa il valore iniettato, per i successivi il risultato
  ...
  def test_all_iteration_methods_work_on_any_collection_not_just_arrays
    ...
    assert_equal [10+1,10+2,10+3], result
  ...
  # UGL aggiunto io, per me l'unica differenza è che con il blocco ci
  #     pensa ruby a chiudere il file alla fine
  def test_file
    # Files act like a collection of lines
